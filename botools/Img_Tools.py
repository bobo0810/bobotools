...
class Img_Tools(object):
    ...
    def plot_bbox(img,bbox,name,prob):
        ...
        for (x, y, w, h),name_i,prob_i in zip(bbox, name, prob):
            ...
            cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA) # 绘制锚框
            cv2.putText(img, name_i+"_"+str(round(prob_i,2)), (c1[0], c1[1] - 2), 0, lw / 3, txt_color, thickness=tl, lineType=cv2.LINE_AA) # 绘制类别概率
        return img
